...
// Defines the JWTs contents, or payload, given a admin object. Make
// changes to what your token looks like here.
function extractPayload(admin, options) {
  console.log(admin)
 return {
  _id: admin._id,
  use: ['admins']
...
/**
* Creates a JWT and returns it to the admin in the response.
...
* 2.  Status 403 Forbidden (when the admin can't be find or password fails)
...
/**
* Creates a new JWT for an authenticated admin (using the authenticate
...
function generateJwt(admin, options) {
 return jwt.sign(
   extractPayload(admin, options),
